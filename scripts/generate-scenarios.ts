...
function groupByCategory(features: FeatureFile[]): Map<string, Scenario[]> {
  ...
  features.forEach((feature) => {
    ...
    const categoryScenarios = groups.get(category);
    if (categoryScenarios) {
      categoryScenarios.push(...feature.scenarios);
    }
  });
...
function generateMarkdown(
  ...
  apiTests.forEach((test) => {
    ...
    const categoryTests = apiByCategory.get(test.category);
    if (categoryTests) {
      categoryTests.push(test);
    }
  });

  for (const [, tests] of Array.from(apiByCategory.entries()).sort()) {
    tests.forEach((test) => {
      const category = test.category
        .split("_")
        .map((w) => `${w.charAt(0).toUpperCase()}${w.slice(1).toLowerCase()}`)
        .join(" ");
      lines.push(`| **${test.id}** | ${category} | ${test.title} |`);
    });
  }
  ...
  // BDD Tests Table
  lines.push("| Status | Scenario | Feature | Type | Tags |");
  lines.push("|---|---|---|---|---|");

  // Sort categories
  for (const [, scenarios] of Array.from(byCategory.entries()).sort()) {
    ...
    for (const scenario of sorted) {
      const status = scenario.isSkipped ? "⏭️ Skipped" : "✅ Active";
      const type = scenario.isOutline ? "Outline" : "Regular";
      const tags = scenario.tags.length
        ? scenario.tags.map((t) => `\`${t}\``).join(" ")
        : "—";
      const featureName = scenario.file.replace(".feature", "");

      lines.push(
        `| ${status} | ${scenario.name} | ${featureName} | ${type} | ${tags} |`,
      );
    }
  }
  ...
  lines.push("- **Outline** — Scenario Outline (parameterized with Examples)");
  lines.push("- **Regular** — Standard Scenario");
  lines.push("");
...
function main() {
  try {
    ...
    const activeScenarios = features.reduce((arr, f) => {
      arr.push(...f.scenarios.filter((s) => !s.isSkipped));
      return arr;
    }, [] as Scenario[]).length;
    const skippedScenarios = features.reduce((arr, f) => {
      arr.push(...f.scenarios.filter((s) => s.isSkipped));
      return arr;
    }, [] as Scenario[]).length;
